...
function defaultInstructions(cwd: string): string {
  return [
    ...
    "",
    "You have these tools: read, write, edit, bash, bash_output, kill_shell, grep, glob, ls.",
    ...
    "",
    "IMPORTANT — long-running commands: any command that does not exit on its own",
    "(dev servers like `npm run dev`/`vite`/`next dev`, watchers, `serve`, `tail -f`)",
    "MUST be run with bash background=true. A foreground bash blocks until it exits and",
    "is killed at the timeout, so never start a server in the foreground. After starting",
    "one, use bash_output(bash_id) to check that it came up (e.g. the dev URL) and report",
    "that URL; use kill_shell(bash_id) to stop it. Use foreground bash only for commands",
    "that finish (builds, tests, installs, git, one-off scripts).",
    "",
